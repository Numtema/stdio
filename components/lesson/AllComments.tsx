...
export function AllComments({ lessons }: { lessons: Lesson[] }) {
  const allComments = lessons.flatMap(l => 
    (l.comments || []).map(c => ({ text: c, lessonTitle: l.title }))
  );
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-8">
      ...
      {allComments.length === 0 ? (
        ...
        <div className="space-y-4">
          {allComments.map((c, i) => (
            <div key={i} className={`p-6 rounded-[24px] ${LIQUID_GLASS} border border-white/5 flex gap-4 items-start`}>
